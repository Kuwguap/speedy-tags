/**
 * Proxy /api/interview/* → krab-interviewer-bot (Vercel serverless).
 * Required because Vercel routes /api/* to the api/ folder before vercel.json rewrites.
 */
const UPSTREAM = (
...
export default async function handler(req, res) {
  ...
  const parts = raw == null ? [] : Array.isArray(raw) ? raw : [String(raw)];
  const subpath = parts.map((p) => String(p).replace(/^\/+|\/+$/g, "")).filter(Boolean).join("/");
