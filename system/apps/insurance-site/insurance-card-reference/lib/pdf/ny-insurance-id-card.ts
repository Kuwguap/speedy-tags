...
export interface NyInsuranceCardInput {
  ...
  /**
   * AAMVA DAQ — customer / document discriminator (e.g. DL document number).
   * Not the policy number. Omit → `000000000` in the PDF417.
   */
  daq?: string
}
...
const TEXT_ITEMS: TxItem[] = [
  ...
  { x: 10.8, y: 748.0, size: 8.64, bold: true, text: d => firstWord(d.issuerCompanyLine ?? '484') },
  { x: 32.4, y: 748.0, size: 8.64, bold: true, text: d => restAfterFirstWord(d.issuerCompanyLine ?? '484 NEW SOUTH INS.CO.') },
  { x: 7.2, y: 730.9, size: 7.2, text: 'Name & Address of Issuer' },
  { x: 97.2, y: 729.2, size: 8.64, bold: true, text: d => d.agencyName ?? 'COYNE INSURANCE AGENCY' },
  { x: 7.2, y: 719.8, size: 7.2, bold: true, text: d => d.issuerPhone ?? '5184772174' },
  { x: 97.2, y: 719.2, size: 8.64, bold: true, text: d => d.agencyAddressLines?.[0] ?? '146 COLUMBIA TURNPIKE' },
  { x: 97.2, y: 709.1, size: 8.64, bold: true, text: d => d.agencyAddressLines?.[1] ?? 'RENSSELAER NY 12144' },
