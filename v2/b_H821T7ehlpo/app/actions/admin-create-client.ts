...
/**
 * Fallback monthly premium when the caller posts `annualPremium: 0` — every
 * policy is priced at $100/mo by default so the dashboard "Balance due" and
 * "Monthly premium" cards render real numbers instead of $0.00. Must match
 * `DEFAULT_MONTHLY_PREMIUM_CENTS` in `lib/supabase/dashboard-data.ts`.
 */
...
function adminPolicyToDashboardRow (args: {
  ...
  const totalCents = Math.max(0, Math.round((args.annualPremium || 0) * 100))
...
async function insertInitialInvoiceForPolicy (
  ...
  try {
    ...
    const { error } = await admin.from('invoices').insert({
      ...
      due_date: dueDate,
      amount_cents: args.monthlyCents,
      status: 'due',
    })
  ...
  if (policyForDashboard) {
    ...
    if (policyId) {
      initialInvoiceWarning = await insertInitialInvoiceForPolicy(admin, {
        ...
        monthlyCents: policyForDashboard.monthly_premium_cents,
    ...
    try {
      ...
      if (resend && from) {
        const { subject, text } = buildPolicyIssuedEmail({
          ...
          loginUrl: 'https://tristatecoverage.com/login',
        })
